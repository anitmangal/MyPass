...
def fenc(psswrd, app, uname, switch, appuser, appold = None, appuserold = None):
    ...
    cursor.execute('SELECT Password FROM {} WHERE App = "{}"'.format(uname, appold))
    psswrd = cursor.fetchone()[0]
    f = Fernet(key.encode())
...
while True:
    ...
    try:
        ...
        print("To proceed further, MyPass requires user's MySQL Database user and password")
        fl = open("data1.txt", "w")
